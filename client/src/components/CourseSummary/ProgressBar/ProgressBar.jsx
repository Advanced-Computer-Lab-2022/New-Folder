...
function ProgressBar(props) {
  ...
  const handleProgressPercentage = async () => {
    ...
    for (let subs = 0 ; subs < subtitles.length ; subs++) {
      ...
      for (let con  = 0 ; con < subContentArr.length ; con++) {
        const content = await FetchContentVisit(subContentArr[con].subTitle_Content_id , subContentArr[con].type);
        if (content) noOftrue++;
      }
  ...
  return (
    <div>
      ...
      {
        done ? (<Progress type="circle" percent={percentage} />)   : (<ReactLoading type={"spin"} color="#fff" />)
      }
    </div>
  );
}
